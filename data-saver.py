...
class DataStorage:
    def __init__(
        ...
        if os.path.exists(self.meta_path):
            ...
            self.df = pd.DataFrame(columns=[
                "uid", "file_path", "style", "emotion", "caption",
    ...
    def embed_pending(self):
        ...
        for i in tqdm(range(0, len(texts), self.batch_size), desc="Эмбеддинг"):
            ...
            self.df.loc[self.df["uid"].isin(batch_uids), "status"] = "ready"
            self.df.loc[self.df["uid"].isin(batch_uids), "vector_key"] = batch_uids
